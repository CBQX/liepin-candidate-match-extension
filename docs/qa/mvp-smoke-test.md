...
- 日期：2026-08-19
- `npm run verify`：通过；TypeScript 类型检查、26 个测试文件 / 368 项测试和生产构建均通过
...
- [ ] 4. 确认岗位画像页允许逐条编辑、增加、删除要求，并调整“硬性/优先/普通”和匹配维度；点击一次“确认岗位画像”后显示“岗位画像已确认”。保存第二个合成岗位并来回切换，确认各自画像不会串用；完全重启 Chrome 后已确认画像仍可直接使用。
...
- [ ] 7. 确认结果包含 0–100 总分、联系建议、可信度、六维评分、要求证据、匹配项、不匹配项、风险、缺失信息、核实问题、沟通建议和猎头结论；联系建议只能是“建议优先联系 / 建议联系 / 建议核实后联系 / 暂不优先联系”，没有淘汰选项。用合成文本分别覆盖地点、工作年限和证书的明确正向、否定、范围、计划或失效表述，确认这三类确定性规则始终为“未知”；即使模型遗漏，相关已脱敏原文仍以“候选人来源证据（需猎头核实）”显示且不含姓名、联系方式、猎聘 URL/ID。比较有无该证据的结果，确认分数、可信度和联系建议不变；再用明确学历样本确认安全结构化规则仍可显示“满足/不满足”。
